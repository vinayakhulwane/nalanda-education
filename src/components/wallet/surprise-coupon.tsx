...
function CouponCard({ coupon, userProfile, recentAttempts = [], worksheets = [], recentTransactions = [], academicHealth }: CouponCardProps) {
    ...
    return (
      <Card className={cn("relative overflow-hidden border-none shadow-xl bg-gradient-to-br from-yellow-50 via-orange-50 to-yellow-100 dark:from-yellow-950/30 dark:to-orange-950/30", justClaimed ? "animate-in fade-in zoom-in duration-500" : "opacity-80 grayscale-[0.3]")}>
          ...
          <CardFooter className="justify-center pb-8 pt-0">
             {justClaimed ? (<Button variant="outline" className="border-yellow-300 bg-yellow-50 hover:bg-yellow-100 text-yellow-800" onClick={() => window.location.reload()}><Sparkles className="mr-2 h-4 w-4" /> Awesome!</Button>) : (<Badge variant="outline" className="border-yellow-600 text-yellow-700 bg-yellow-50 px-4 py-1">Reward Collected</Badge>)}
          </CardFooter>
      </Card>
    );
  }
...
export function SurpriseCoupon({ userProfile }: SurpriseCouponProps) {
  ...
  useEffect(() => {
    const fetchWorksheets = async () => {
        ...
        const chunks = []; for (let i = 0; i < allIds.length; i += 30) chunks.push(allIds.slice(i, i + 30));
        try {
            const results: Worksheet[] = [];
            for (const chunk of chunks) { if (chunk.length === 0) continue; const q = query(collection(firestore, 'worksheets'), where(documentId(), 'in', chunk)); const snap = await getDocs(q); snap.forEach(doc => results.push({ id: doc.id, ...doc.data() } as Worksheet)); }
    ...
    const getRank = (c: Coupon) => {
        ...
        if (c.conditions) {
           ...
           for (const cond of c.conditions) {
              ...
              let count = 0;
              if (cond.type === 'minClassroomAssignments') count = validAttempts.filter(a => check((worksheets || []).find(w => w.id === a.worksheetId), a, 'classroom')).length;
              else if (cond.type === 'minPracticeAssignments') count = validAttempts.filter(a => check((worksheets || []).find(w => w.id === a.worksheetId), a, 'practice')).length;
              else if (cond.type === 'minGoldQuestions') count = validTransactions.filter(t => t.currency?.toLowerCase() === 'gold' && t.type === 'earned').length;
              else if (cond.type === 'minAcademicHealth') { if (academicHealth < cond.value) { tasksDone = false; break; } continue; }
  ...
  }, [coupons, userProfile, recentAttempts, worksheets, recentTransactions, academicHealth]);
  ...
  if(!sortedCoupons || sortedCoupons.length === 0) return (<Card className="text-center p-8 border-dashed"><CardHeader><CardTitle>No Coupons Available</CardTitle></CardHeader><CardContent><p className="text-muted-foreground">Check back later for new rewards and promotions!</p></CardContent></Card>);

  return (
    <div className="grid gap-6 md:grid-cols-2">
      {sortedCoupons.map(coupon => (
        <CouponCard key={coupon.id} coupon={coupon} userProfile={userProfile} recentAttempts={recentAttempts ?? []} worksheets={worksheets ?? []} recentTransactions={recentTransactions ?? []} academicHealth={academicHealth} />
      ))}
    </div>
  )
}
