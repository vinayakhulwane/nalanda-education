...
const getAttemptTotals = (a: WorksheetAttempt, worksheet: Worksheet | undefined, allQuestions: Map<string, any>) => {
    ...
    if (typeof savedScore === 'number' && typeof savedTotal === 'number' && savedTotal > 0) {
        if (savedScore > savedTotal) return { score: (savedScore / 100) * savedTotal, total: savedTotal };
    ...
    if (worksheet) {
        worksheet.questions.forEach(qId => {
            ...
            if (question) {
                ...
                if (qEarned > qMax && qMax > 0) qEarned = (qEarned / 100) * qMax;
                calcScore += qEarned;
            }
        });
    }
...
export function StudentProgressDetail({ student }: StudentProgressDetailProps) {
  ...
  const enrolledSubjectsQuery = useMemoFirebase(() => {
    ...
    return query(collection(firestore, 'subjects'), where(documentId(), 'in', student.enrollments.slice(0, 30)));
  }, [firestore, student.enrollments]);
  ...
  const attemptsQuery = useMemoFirebase(() => {
    ...
    return query(collection(firestore, 'worksheet_attempts'), where('userId', '==', student.id), orderBy('attemptedAt', 'asc'));
  }, [student, firestore]);
  ...
  const analytics = useMemo(() => {
    ...
    attempts.forEach(attempt => {
        ...
        if (attempt.attemptedAt) {
             ...
             d.total += data.total; d.obtained += data.score;
        }
        worksheet.questions.forEach(qId => {
            const question = questionMap.get(qId);
            if (!question) return;
            let qMax = 0; let qScore = 0;
            question.solutionSteps?.forEach((step: any) => {
                step.subQuestions.forEach((sub: any) => {
                    const m = sub.marks || 0;
                    qMax += m;
            ...
            if (targetUnitId && activeUnitIds.has(targetUnitId)) {
                ...
                if (m) { m.attemptCount++; m.totalMarks += qMax; m.obtainedMarks += qScore; }
            }
            if (question.categoryId) {
                 ...
                 if (c && c.unitId && activeUnitIds.has(c.unitId)) {
                     ...
                     mCat.attemptCount++; mCat.totalMarks += qMax; mCat.obtainedMarks += qScore;
                 }
            }
        });
    });
    metricsMap.forEach(m => { if (m.totalMarks > 0) m.percentage = Math.min(100, (m.obtainedMarks / m.totalMarks) * 100); });
    let currentStreak = 0;
    ...
    if (sortedDates.length > 0) {
        ...
        if (last === today || last === yesterday) {
            ...
            for (let i = sortedDates.length - 2; i >= 0; i--) {
                if (differenceInDays(new Date(sortedDates[i+1]), new Date(sortedDates[i])) === 1) currentStreak++; else break;
            }
        }
    }
  ...
  const isLoading = subjectsLoading || attemptsLoading || !lookups;
  if (!selectedSubjectId) {
    return (
        <Card className="mt-6">
          <CardHeader>
            <CardTitle>Subject Progress</CardTitle>
            <CardDescription>Select a subject to view detailed progress.</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {subjects && subjects.length > 0 ? subjects.map((subject) => {
                      ...
                      return (
                        <Card key={subject.id}>
                          <CardHeader className='pb-2'><CardTitle className="text-base">{subject.name}</CardTitle></CardHeader>
                          <CardContent><div className="flex justify-between text-sm"><span className="text-muted-foreground">Health</span><span className="font-bold">{health}%</span></div><Progress value={health} className={cn("h-2 mt-1", healthColor)} /></CardContent>
                          <CardFooter><Button variant="secondary" className="w-full" onClick={() => setSelectedSubjectId(subject.id)}>View Details</Button></CardFooter>
                        </Card>
                      );
                    }) : <p className="text-muted-foreground text-center col-span-full py-10">This student is not enrolled in any subjects.</p>}
                </div>
            )}
          </CardContent>
        </Card>
  ...
  if (!analytics) return <div className="flex h-64 items-center justify-center"><Loader2 className="h-8 w-8 animate-spin" /></div>;
  ...
  const InsightGrid = ({ data }: { data: typeof analytics.unitMetrics }) => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-500">
        <Card className="border-l-4 border-l-green-500 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-lg"><TrendingUp className="h-5 w-5 text-green-600" /> Strengths</CardTitle><CardDescription>Score &ge; 75%</CardDescription></CardHeader><CardContent>{data.strengths.length > 0 ? (<div className="space-y-4">{data.strengths.slice(0, 5).map(s => (<div key={s.id} className="flex justify-between items-start"><div className="flex flex-col truncate w-2/3"><span className="text-sm font-medium">{s.name}</span><div className="flex items-center gap-1 text-[10px] text-muted-foreground mt-0.5"><span className="uppercase font-bold tracking-wider">{s.type}</span>{s.type === 'category' && s.parentName && (<><span>•</span><span className="truncate">{s.parentName}</span></>)}</div></div><Badge className="bg-green-100 text-green-700 border-none">{Math.round(s.percentage)}%</Badge></div>))}</div>) : <div className="text-sm text-muted-foreground py-4 text-center">No clear strengths yet.</div>}</CardContent></Card>
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between">
        <div>
          <CardTitle>Detailed Progress: {selectedSubjectName}</CardTitle>
          <CardDescription>Performance metrics for this subject.</CardDescription>
        </div>
      ...
      <CardContent className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-gradient-to-br from-orange-50 to-white border-orange-200"><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><Flame className="h-4 w-4 text-orange-500" /> Streak</CardTitle></CardHeader><CardContent><div className="text-4xl font-bold text-orange-600 flex items-baseline gap-2">{analytics.streak} <span className="text-lg font-normal text-muted-foreground">days</span></div></CardContent></Card>
          <Card className="bg-gradient-to-br from-blue-50 to-white border-blue-200"><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><TrendingUp className="h-4 w-4 text-blue-500" /> Health</CardTitle></CardHeader><CardContent><div className="text-4xl font-bold text-blue-600 flex items-baseline gap-2">{analytics.currentHealth}%</div></CardContent></Card>
          <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><Trophy className="h-4 w-4 text-yellow-500" /> Worksheets</CardTitle></CardHeader><CardContent><div className="text-4xl font-bold text-foreground">{analytics.activeAttemptsCount}</div></CardContent></Card>
        </div>
        <Card>
